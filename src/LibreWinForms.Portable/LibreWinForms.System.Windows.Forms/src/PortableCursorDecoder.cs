...
internal static class PortableCursorDecoder
{
    ...
    public static Bitmap DecodeFile(string fileName)
    {
        ...
        if (stream.Length > MaximumCursorFileBytes)
        {
            throw new InvalidDataException(
                $"Cursor file exceeds the {MaximumCursorFileBytes}-byte portable decode limit.");
        }

        if (stream.Length < CursorDirectoryHeaderSize)
        {
            throw new InvalidDataException("Cursor file is truncated before its directory header.");
        }
    ...
    private static Bitmap Decode(byte[] container)
    {
        ReadOnlySpan<byte> bytes = container;
        ushort reserved = BinaryPrimitives.ReadUInt16LittleEndian(bytes);
        ushort type = BinaryPrimitives.ReadUInt16LittleEndian(bytes[2..]);
        ushort count = BinaryPrimitives.ReadUInt16LittleEndian(bytes[4..]);
        if (reserved != 0 || type != 2)
        {
            throw new InvalidDataException("The file is not a Windows cursor container (CUR type 2).");
        }

        if (count == 0 || count > MaximumCursorEntries)
        {
            throw new InvalidDataException(
                $"Cursor directory entry count must be between 1 and {MaximumCursorEntries}.");
        }

        int directorySize = checked(CursorDirectoryHeaderSize + count * CursorDirectoryEntrySize);
        if (directorySize > bytes.Length)
        {
            throw new InvalidDataException("Cursor directory entries extend beyond the file bounds.");
        }
        ...
        for (int index = 0; index < count; index++)
        {
            ...
            ReadOnlySpan<byte> encodedEntry = bytes.Slice(entryOffset, CursorDirectoryEntrySize);
            ...
            if (encodedOffset < directorySize || payloadEnd > bytes.Length)
            {
                throw new InvalidDataException($"Cursor directory entry {index} points outside the file bounds.");
            }
            ...
            CursorPayloadKind kind = ClassifyPayload(payload);
            if (kind == CursorPayloadKind.Unsupported)
            {
                continue;
            }

            var candidate = new CursorEntry(width, height, payloadOffset, payloadLength, kind, index);
        ...
        if (selected is null)
        {
            throw new NotSupportedException(
                "Cursor contains no supported PNG or uncompressed 32-bpp DIB image payload.");
        }

        return selected.Value.Kind switch
        {
            CursorPayloadKind.Png => DecodePng(container, selected.Value),
            CursorPayloadKind.Dib => DecodeDib(bytes, selected.Value),
    ...
    {
        if (payload.Length >= s_pngSignature.Length
            && payload[..s_pngSignature.Length].SequenceEqual(s_pngSignature))
        {
            return CursorPayloadKind.Png;
        }

        if (payload.Length >= sizeof(uint)
            && BinaryPrimitives.ReadUInt32LittleEndian(payload) >= BitmapInfoHeaderSize)
        {
    ...
    private static Bitmap DecodePng(byte[] container, CursorEntry entry)
    {
        ReadOnlySpan<byte> payload = container.AsSpan(entry.PayloadOffset, entry.PayloadLength);
    ...
    private static Bitmap DecodeDib(ReadOnlySpan<byte> container, CursorEntry entry)
    {
        ...
        {
            throw new NotSupportedException(
                $"Cursor DIB bit depth {bitsPerPixel} is not supported; only uncompressed 32-bpp images are supported.");
        }

        if (compression != 0)
        {
            throw new NotSupportedException(
                $"Cursor DIB compression mode {compression} is not supported; only BI_RGB is supported.");
        }
    ...
    private static void ValidateImageDimensions(uint width, uint height, CursorEntry entry)
    {
        ...
        {
            throw new InvalidDataException(
                $"Cursor image exceeds the {MaximumDecodedPixels}-pixel portable decode limit.");
        }
