...
The advanced formatted-text slices carry `StringFormat` direction, tab-stop, trailing-space, native-digit, no-font-fallback, visible format-control, and mnemonic state through the same typed ProGPU text and retained-scene paths. Nine additional focused cases raise the complete local drawing suite to 147/147 and the formatted-text focused class to 14/14. `DisplayFormatControl` now proves that the preserved default ignorable records an outlined representative glyph, while `HotkeyPrefix.Show` records one brush-, transform-, clip-, face-, and cluster-aware underline and `Hide` only performs ampersand unescaping. The local API gate remains at the reviewed 55 missing types, 319 missing members, 47 other diagnostics, and 421 total with no stale or breaking suppressions. On ARM64 .NET 10 in-process ShortRuns, `MeasureSpan` recorded an 11.909 microsecond median at 6.64 KB/op, `MeasureAdvancedFormatSpan` recorded a 7.235 microsecond median at 5.67 KB/op, and `RecordMnemonicString` recorded a 3.021 microsecond median at 2.02 KB/op. Exact `LineLimit`, `EllipsisPath`, vertical mnemonic decoration, and representative vertical/RTL pixel baselines remain explicit follow-up work.
