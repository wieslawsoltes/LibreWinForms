...
The headless integration gate calls unchanged `Application.Run(form)` and verifies ordered `HandleCreated`, `VisibleChanged`, asynchronously marshaled `Shown`, form and child layout, form bounds, full-client invalidation, `Update()` routing, both canonical Paint callbacks, clip and visible-surface bounds, translated form/child pixel colors, close cancellation/retry, `FormClosed`, and `HandleDestroyed` behavior, dispatcher termination, disposal, and zero leaked handles. This proves the main-form lifecycle plus a canonical source-built ProGPU paint frame on Linux. Owned/modal forms, dirty-region retention, `CreateGraphics`, real display presentation, representative stock-control rendering, and canonical input dispatch are still required before Phase 3/4 exit.
