...
`src/LibreWinForms.ProGPU` supplies the first concrete backend foundation. It includes an owning-thread dispatcher with nested loops and synchronous/asynchronous marshaling, dispatcher-delivered timers, typed Silk.NET top-level windows, ProGPU `SilkWindowController` integration, keyboard/text/pointer/focus normalization, close cancellation, explicit handle release, and dispatcher-safe coalesced paint scheduling. Silk keys are translated explicitly to the backend-neutral `LibreKey` contract instead of leaking Silk enum integers, and pointer/wheel events carry the current normalized keyboard modifiers. Each Silk window now owns a `WgpuContext`, `Compositor`, and retained `DrawingVisual`; it records a typed `System.Drawing.Graphics` frame, acquires/reconfigures the WebGPU surface, composites logical pixels at the window DPI, and presents with bounded retry for transient surface loss/timeouts. Headless tests cover registration, handle type/kind/lifetime rules, concurrent allocation, monitor selection, dispatch ordering, cross-thread send, timer affinity, and concrete service composition. Silk monitor enumeration is connected through its public typed API. Silk.NET 2.x does not expose work area, monitor color depth, or physical DPI, so that adapter truthfully reports bounds as work area, 32-bit compositor format, and neutral monitor scale; created windows still report framebuffer/native window scale through `ILibreWindow.DpiScale`. Real display-server presentation/input/monitor smokes on every target OS remain open.
...
The GPU-independent headless integration gate calls unchanged `Application.Run(form)` and verifies ordered `HandleCreated`, `VisibleChanged`, asynchronously marshaled `Shown`, form and child layout, form bounds, full-client invalidation, `Update()` routing, both canonical Paint callbacks, clip and visible-surface bounds, exact retained solid-fill rectangles/RGBA values for the form and translated child, close cancellation/retry, `FormClosed`, and `HandleDestroyed` behavior, dispatcher termination, disposal, and zero leaked handles. It also drives normalized focus, pointer move/down/up/wheel, key down/text/key up, and focus loss through the real top-level `NativeWindow`; verifies logical child hit testing and client coordinates; and observes canonical `GotFocus`, mouse, click, wheel, `KeyDown`, `KeyPress`, `KeyUp`, and `LostFocus` events with portable `Focused`, `ContainsFocus`, `Capture`, `MouseButtons`, `MousePosition`, and `ModifierKeys` state. `ContainerControl.ActiveControl` focus activation now has a managed portable path rather than falling back to USER32. A second unchanged-API scenario shows a non-modal owned tool window, an explicitly owned first dialog, and an implicitly owned nested dialog; it verifies typed native ownership, public-versus-platform enabled state, input suppression, nested restoration order, dialog results, modal flags, owner activation, and zero leaked handles. A third scenario validates canonical monitor/screen/system-information and managed form-centering behavior across a two-monitor virtual desktop. This proves the main-form lifecycle, initial canonical input crossing, same-dispatcher owned/nested-modal behavior, initial monitor routing, and a canonical source-built context-backed ProGPU paint frame on Linux without requiring a display adapter. Real framebuffer pixel validation remains part of the display-server presentation gate. Multi-dispatcher ownership, dynamic monitor/DPI propagation, dirty-region retention, `CreateGraphics`, real display presentation, representative stock-control rendering/input, keyboard command/dialog preprocessing, mnemonics/tab traversal, double-click/context-menu behavior, IME, and platform input/modal smokes are still required before Phase 3/4 exit.
...
Current status: canonical full-client and rectangular invalidation translate logical child coordinates to the top-level window and reach the typed paint scheduler; `Update()` reaches its presentation seam. The backend now owns a real ProGPU/WebGPU window surface and canonical `PaintEventArgs.Graphics` records form and logical-child painting into a retained frame with DPI-aware presentation. The GPU-independent headless gate checks exact form and translated-child retained fill coordinates and RGBA values. A backend-neutral `LibreKey` contract and managed canonical dispatcher now cover initial focus, logical-tree pointer hit testing, capture/button/modifier state, move/down/up/click/wheel events, and key down/text/key up events. Typed monitor enumeration/selection, canonical `Screen`, monitor-derived `SystemInformation`, and form centering are connected and headless-tested. Dynamic monitor changes, DPI propagation/rescaling, remaining system metrics, region invalidation, retained dirty-region/backing-store semantics, synchronous presentation semantics, `CreateGraphics`, stock-control HDC substitutions, real display-server/pixel/input smokes, and the advanced input cases below remain open.
