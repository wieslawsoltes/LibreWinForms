...
The candidate source checkpoint pins ProGPU commit `5c5345609725318c5af80ba7ddd9e2bd6bdbbe20`, which integrates the formatted-text slice with current ProGPU `main`. The preceding exact source checkpoint passed the platform contracts (20/20), ProGPU host contracts (8/8), canonical lifecycle/retained-paint/input plus owned/nested-modal, multi-monitor, logical presentation-scale, and per-monitor-V2 DPI integration (7/7), and ProGPU drawing quality suite (133/133). The replacement adds five formatted-text span/range quality cases, all passing in-process; hosted exact-head execution and source-first package validation are running. The hosted Linux lane provisions Mesa's software Vulkan adapter because those quality cases execute the production typed WebGPU recording/readback path. The canonical portable build remains at its reviewed 619-warning/zero-error baseline. The preceding hosted package contained the canonical implementation/reference assets, declared `ProGPU.System.Drawing.Common` as a normal dependency, did not embed ProGPU assemblies, and passed a fresh-cache consumer build with warnings treated as errors. Its hashes are retained in the prior checkpoint history and will be superseded by artifacts from the new exact pin.
