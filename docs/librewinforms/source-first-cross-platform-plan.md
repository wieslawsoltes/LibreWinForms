...
`src/LibreWinForms.ProGPU` supplies the first concrete backend foundation. It includes an owning-thread dispatcher with nested loops and synchronous/asynchronous marshaling, dispatcher-delivered timers, typed Silk.NET top-level windows, ProGPU `SilkWindowController` integration, keyboard/text/pointer/focus normalization, close cancellation, explicit handle release, and coalesced paint scheduling. Headless tests cover registration, handle type/kind/lifetime rules, concurrent allocation, dispatch ordering, cross-thread send, timer affinity, and concrete service composition. Silk monitor enumeration and actual ProGPU paint-surface projection remain explicit typed `PlatformNotSupportedException`/next-slice boundaries.

The canonical lifecycle now selects those contracts under `LIBREWINFORMS_PORTABLE`. Canonical `Application.ThreadContext` and `WindowsFormsSynchronizationContext` use the registered dispatcher; `NativeWindow` maps top-level forms and logical child controls to opaque typed handles; and canonical `Control`/`Form` retain their public lifecycle while routing create, visibility, close, marshaled callbacks, and destruction through the backend. Portable initialization no longer enters eager USER32/GDI DPI, message-registration, comctl32, or window-style paths. A headless integration gate calls unchanged `Application.Run(form)` and verifies ordered `HandleCreated`, `VisibleChanged`, asynchronously marshaled `Shown`, close cancellation/retry, `FormClosed`, and `HandleDestroyed` behavior, dispatcher termination, disposal, and zero leaked handles. This proves the main-form lifecycle slice on Linux; owned forms, modal loops, real Silk windows, painting, and input are still required before Phase 3 exits.
...
Current status: the unchanged main-form run/show/close/shutdown path is implemented and headless-tested through canonical source. Owned/modal forms, production Silk execution, idle/exception coverage, and full platform-matrix validation remain open.
