...
The verified source checkpoint pins ProGPU commit `612fbc3ea5f6eac26160022e85403664803dfbd5`. A clean source-first run passes the platform contracts (7/7), ProGPU host contracts (8/8), canonical lifecycle/retained-paint/input plus owned/nested-modal, multi-monitor, and presentation-scale integration (4/4), and ProGPU drawing quality suite (82/82). The canonical portable build remains at its reviewed 619-warning/zero-error baseline. A fresh-cache package consumer builds with warnings treated as errors; the packed `System.Windows.Forms.dll` SHA-256 is `5dd1418a574e1af550f913f2b0c0f3781f6ef70da154da12885b3a62d62e22ac`, `LibreWinForms.Platform.dll` SHA-256 is `5be309ea6d053a312b9c1a872f4625a1462706d3a9680868102bbb7a60f60ed2`, and the source-first package SHA-256 is `4ae0496fa14e6968df42196e84a9dcd34f18c7ff9c7a5d47772c178c7927deb3`.
...
The GPU-independent headless integration gate calls unchanged `Application.Run(form)` and verifies ordered `HandleCreated`, `VisibleChanged`, asynchronously marshaled `Shown`, form and child layout, form bounds, full-client invalidation, `Update()` routing, both canonical Paint callbacks, clip and visible-surface bounds, exact retained solid-fill rectangles/RGBA values for the form and translated child, close cancellation/retry, `FormClosed`, and `HandleDestroyed` behavior, dispatcher termination, disposal, and zero leaked handles. It also drives normalized focus, pointer move/down/up/wheel, key down/text/key up, and focus loss through the real top-level `NativeWindow`; verifies logical child hit testing and client coordinates; and observes canonical `GotFocus`, mouse, click, wheel, `KeyDown`, `KeyPress`, `KeyUp`, and `LostFocus` events with portable `Focused`, `ContainsFocus`, `Capture`, `MouseButtons`, `MousePosition`, and `ModifierKeys` state. `ContainerControl.ActiveControl` focus activation now has a managed portable path rather than falling back to USER32. A second unchanged-API scenario covers non-modal and nested-modal ownership. A third validates monitor/screen/system-information and centering across two monitors. A fourth drives a 1x-to-2x presentation-scale change through the typed host, verifies full-surface invalidation, and proves that logical form bounds and virtualized 96-DPI `DeviceDpi` remain stable rather than double-scaling. This proves the main-form lifecycle, initial canonical input crossing, same-dispatcher owned/nested-modal behavior, initial monitor and presentation-scale routing, and a canonical source-built context-backed ProGPU paint frame on Linux without requiring a display adapter. Real framebuffer pixel validation remains part of the display-server presentation gate. Multi-dispatcher ownership, dynamic monitor inventory propagation, an opt-in device-pixel/per-monitor autoscale mode, dirty-region retention, `CreateGraphics`, real display presentation, representative stock-control rendering/input, keyboard command/dialog preprocessing, mnemonics/tab traversal, double-click/context-menu behavior, IME, and platform input/modal smokes are still required before Phase 3/4 exit.
