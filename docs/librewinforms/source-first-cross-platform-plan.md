...
The verified source checkpoint pins ProGPU commit `612fbc3ea5f6eac26160022e85403664803dfbd5`. A clean source-first run passes the platform contracts (4/4), ProGPU host contracts (4/4), unchanged canonical `Application.Run(Form)` lifecycle/retained-paint integration (1/1), and ProGPU drawing quality suite (82/82). The canonical portable build remains at its reviewed 619-warning/zero-error baseline. A fresh-cache package consumer builds with warnings treated as errors; the packed `System.Windows.Forms.dll` SHA-256 is `95bdebcb42aa682f4792cb3533aa983ab49b552169c1bd7e25ebbeeeead53b6f` and `LibreWinForms.Platform.dll` SHA-256 is `323d014faa8ab3ded752b1ccfcf74b2d17a6e8d0ca28733e4b059c45b3dc68ea`.
...
The GPU-independent headless integration gate calls unchanged `Application.Run(form)` and verifies ordered `HandleCreated`, `VisibleChanged`, asynchronously marshaled `Shown`, form and child layout, form bounds, full-client invalidation, `Update()` routing, both canonical Paint callbacks, clip and visible-surface bounds, exact retained solid-fill rectangles/RGBA values for the form and translated child, close cancellation/retry, `FormClosed`, and `HandleDestroyed` behavior, dispatcher termination, disposal, and zero leaked handles. This proves the main-form lifecycle plus a canonical source-built context-backed ProGPU paint frame on Linux without requiring a display adapter. Real framebuffer pixel validation remains part of the display-server presentation gate. Owned/modal forms, dirty-region retention, `CreateGraphics`, real display presentation, representative stock-control rendering, and canonical input dispatch are still required before Phase 3/4 exit.
...
- Implement keyboard, text/IME, pointer, wheel, focus, capture, tab navigation, mnemonics, and command keys.
