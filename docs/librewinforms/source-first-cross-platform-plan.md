...
The current candidate pins ProGPU merge commit `142b1af7ca339e17cc0bf5dbee9fc2c20eacd6ef`. That commit has exact parents `02096ecf9c99ba09d678dd7da3ad2bb6bc3457ac` (the advanced formatted-text source slice) and `ad05be151a8e844ccf0abe496a12b29124568e6b` (the then-current ProGPU `main`, versioned `0.1.0-preview.57`); every one of the 23 files brought in from that `main` advance was blob-hash verified against GitHub. The preceding hosted source-first checkpoint at ProGPU `5c5345609725318c5af80ba7ddd9e2bd6bdbbe20` passes the platform contracts (20/20), ProGPU host contracts (8/8), canonical lifecycle/retained-paint/input plus owned/nested-modal, multi-monitor, logical presentation-scale, and per-monitor-V2 DPI integration (7/7), and the then-complete ProGPU drawing quality suite (138/138). The hosted Linux lane provisions Mesa's software Vulkan adapter because those quality cases execute the production typed WebGPU recording/readback path. The canonical ProGPU-backed build remains at its reviewed 613-warning/zero-error baseline. The hosted package contains the canonical implementation/reference assets, declares `ProGPU.System.Drawing.Common` as a normal dependency, does not embed ProGPU assemblies, and passes a fresh-cache consumer build with warnings treated as errors; the packed `System.Windows.Forms.dll` SHA-256 is `ebb09d23a2cbc460d55114100f3b891f7b9cf46ec073176a90b0a77289171ae8`, the packed `LibreWinForms.Platform.dll` SHA-256 is `d8beb6fd36bdb5346e8789f117628a9f232b5f7ca6b50369ce703df1acfd025c`, and the uploaded `canonical-source-first-package` artifact SHA-256 digest is `dfa5fac937a1a09f89e0985c8fbdf952c4374f258e0f306cdd759ba67143ab6c`. The hosted lane must be rerun at the new candidate before those package hashes are promoted to evidence for the new pin.
...
The advanced formatted-text slice carries `StringFormat` direction, tab-stop, trailing-space, native-digit, and no-font-fallback state through the same typed ProGPU text layout path. Six additional focused cases raise the complete local drawing suite to 144/144 and the advanced-format focused class to 11/11. The local API gate remains at the reviewed 55 missing types, 319 missing members, 47 other diagnostics, and 421 total with no stale or breaking suppressions. On the paired ARM64 .NET 10 in-process ShortRun, `MeasureSpan` recorded an 11.909 microsecond median at 6.64 KB/op and `MeasureAdvancedFormatSpan` recorded a 7.235 microsecond median at 5.67 KB/op. Visible `DisplayFormatControl` glyphs, hotkey underline rendering, exact `LineLimit`, `EllipsisPath`, and representative vertical/RTL pixel baselines remain explicit follow-up work.
