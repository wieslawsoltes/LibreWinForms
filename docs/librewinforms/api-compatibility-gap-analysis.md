...
ProGPU now has a pinned, suppression-controlled ApiCompat gate against `Microsoft.WindowsDesktop.App.Ref` 10.0.11. Its current reviewed result is 69 missing types, 460 missing members, 47 other shape diagnostics, and 576 total diagnostics, down from 1,052 total in this report's original audit. Missing-type reductions can expose member diagnostics that were previously hidden, so subsystem completion and suppression diffs matter more than any one subtotal.
