...
The first proposed source-first slice is now implemented on the feature branches. The canonical `System.Windows.Forms` project can opt into source-project references to the ProGPU drawing stack without changing its `System.Windows.Forms` assembly or namespace identity. The ordinary NuGet/package reference mode remains the default, and the existing portable implementation remains temporarily available as a comparison lane.

ProGPU now has a pinned, suppression-controlled ApiCompat gate against `Microsoft.WindowsDesktop.App.Ref` 10.0.11. Its current reviewed result is 69 missing types, 464 missing members, 47 other shape diagnostics, and 580 total diagnostics, down from 1,052 total in this report's original audit. Missing-type reductions can expose member diagnostics that were previously hidden, so subsystem completion and suppression diffs matter more than any one subtotal.

Implemented drawing groups include complete known-color brush/pen properties with allocation-free warmed caches, retained `Region` boolean geometry and clipping, a functional allocation-free warmed affine `Matrix` layer, functional `Blend`/`ColorBlend`/`LinearGradientBrush` state and typed rendering lowering, and retained `GraphicsPath` plus `GraphicsPathIterator` contracts with source-compatible point/type data and traversal, cardinal curves, cloning/composition, transforms, analytic bounds, fill and outline hit-testing, widening, perspective/bilinear warping, reversal, and adaptive flattening. Stroke expansion and path deformation are reusable typed `ProGPU.Vector` services with behavior, allocation, ApiCompat, and BenchmarkDotNet gates. Additional `Graphics`/image/font/icon APIs required by canonical WinForms, functional `ColorMap` remapping and defensive `ImageAttributes` state, clone-safe palette/property metadata, deterministic fixed and CPU-only optimal palette generation, CPU-only image resolution/tag/frame/bounds contracts, a managed buffered-graphics model, and a managed printing/controller/event model whose unavailable native operations fail at explicit platform boundaries are also present. Canonical source builds now exercise these APIs directly. Remaining ProGPU incompatibilities and WinForms platform seams are recorded in the source-first plan and generated ApiCompat artifact rather than being filled with compatibility-only stubs.
