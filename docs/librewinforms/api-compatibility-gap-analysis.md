...
The coordinated drawing dependency is [ProGPU draft PR #140](https://github.com/wieslawsoltes/ProGPU/pull/140), pinned by the LibreWinForms submodule at exact commit `d7129e2ef40737b2c068310ed8e458561e8908e8`. That commit descends from the merge of ProGPU `main` commit `812ab6dc49b6630471c21a837409f1152f6110bd` (preview 60) while retaining source-project development mode and the normal NuGet/package mode. ProGPU now has a suppression-controlled ApiCompat gate against `Microsoft.WindowsDesktop.App.Ref` 10.0.11. Its current reviewed result is 50 missing types, 319 missing members, 47 other shape diagnostics, and 416 total diagnostics, down from 1,052 total in this report's original audit, with no breaking changes or stale suppressions. Missing-type reductions can expose member diagnostics that were previously hidden, so subsystem completion and suppression diffs matter more than any one subtotal.
