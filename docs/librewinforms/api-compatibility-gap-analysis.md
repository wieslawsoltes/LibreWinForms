...
The next platform candidate also makes canonical `Control.Update()` synchronous at the typed presentation boundary: pending paint is delivered on the owning dispatcher and presented before return, while a clean `Update()` does not manufacture invalidation or another `OnPaint`. A direct ProGPU host test covers cross-thread dispatcher completion and the canonical headless lifecycle gate covers callback ordering and clean-window behavior. This changes no `System.Drawing` API debt and keeps the normal package graph independent of the development submodule mode.
