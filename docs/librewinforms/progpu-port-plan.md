...
Hosted text retains bounded frozen WPF drawing groups after formatting, so stable labels reuse their typed glyph drawing payloads instead of expanding the same `FormattedText` into fresh render records on every invalidated host frame. The package-mode allocation gate verifies both the steady-frame budget and bounded cache release after the hosted tree is detached.
